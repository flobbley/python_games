...
def getWordScore(word, n):
    ...
    base_score = 0
    for letter in word:
        base_score += SCRABBLE_LETTER_VALUES[letter]
    base_score *= len(word)
    if len(word) == n:
        base_score += 50
    return base_score
...
def isValidWord(word, hand, wordList):
    ...
    word_test = hand.copy()
    for letter in word:
        if letter not in word_test:
            hand_valid = False
            break
        else:
            word_test[letter] -= 1
            hand_valid = all(x>=0 for x in word_test.values())
    word_valid = word in wordList
    if word_valid and hand_valid:
        valid = True
    return valid
    ...
    while calculateHandlen(hand)>0: # As long as there are still letters left in the hand:
        ...
        if word == '.':# If the input is a single period:
            ...
            if isValidWord(word, hand, wordList) == False: # If the word is not valid:
                ...
                round_score = getWordScore(word, n)
                total_score += round_score # Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                print('"'+word+'" earned',round_score,'points. Total: ',total_score,'points')
                print()
                hand = updateHand(hand,word)# Update the hand 
                
    if calculateHandlen(hand) == 0:
        print('Run out of letters. Total score:',total_score,'points')# Game is over (user entered a '.' or ran out of letters), so tell user the total score
    elif word == '.':
...
def playGame(wordList):
    ...
    still_playing = True
    while still_playing:
        game_type = input('Enter n to deal a new hand, r to replay the last hand, or e to end the game:')
        if game_type == 'n':
